...
function KonvaCanvas() {
    ...
    const handleDragMove = (e: Konva.KonvaEventObject<DragEvent>, el: ElementProperties) => {
        ...
        if ((el.type === "rectTable" || el.type === "circleTable") && el.attachedChairs!.length > 0) {
            ...
            const updates = el.attachedChairs!.map((chairId: string) => {
    ...
    const handleDragEnd = (e: Konva.KonvaEventObject<DragEvent>, el: ElementProperties) => {
        ...
        if (el.type === "chair") {
            ...
            const tables = state.elements.filter((t): t is Table =>
                t.type === "circleTable" || t.type === "rectTable"
            );

            const table = tables.find(t => {
                return (
                    (t.type === "circleTable" &&
                        Math.hypot(t.x - x, t.y - y) < t.radius! + el.radius!) ||
                    (t.type === "rectTable" &&
                        x >= t.x - el.radius! &&
    ...
    const handleTransformEnd = () => {
        ...
        for (const node of nodes) {
            ...
            const updates = el.type === "circleTable" || el.type === "chair" ? {
                id,
    ...
    const handleMouseDown = (e: { evt: MouseEvent, target: Konva.Node }) => {
        ...
        if (state.buildMode === 0) {
            ...
            if (quickWallCoordinates.x1 === undefined) {
                setQuickWallCoordinates({x1: (pointer.x - stage.x()) / scale, y1: (pointer.y - stage.y()) / scale});
            } else {
    ...
    const handleMouseUp = () => {
        ...
        const selected = state.elements.filter(rect => {
            ...
            return Konva.Util.haveIntersection(selBox, rectRefs.current.get(rect.id).getClientRect({relativeTo: stageRef.current}));
        });
    ...
    return (
        <div className="App" style={{display: "flex", flexDirection: "row"}}>
            ...
            {selectedIds.length === 1 && (
                <ElementInspector dispatch={dispatch} state={state} selectedIds={selectedIds}
                                  setSelectedIds={setSelectedIds}></ElementInspector>
            )}
        </div>
    );
