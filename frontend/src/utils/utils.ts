...
export const exportParticipationToCSV = (data: ParticipationDetails[], eventName: string) => {
  ...
  // Define CSV headers
  const headers = ["Gitlab ID", "Name", "Last Name", "Email", "Guest Count"];
...
export const exportDietaryPreferencesToCSV = (
  ...
  // Define CSV headers
  const headers = ["Dietary Preference", "Count"];
  // Map data to CSV rows
  const rows = sortedEntries.map(([preference, count]) => [preference, count.toString()]);
  // Combine headers and rows
  const csvContent = [headers, ...rows].map(e => e.join(";")).join("\n");
  // Create a blob and trigger download
  const blob = new Blob([csvContent], { type: "text/csv;charset=utf-8;" });
